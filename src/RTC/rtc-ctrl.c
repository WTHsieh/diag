...
RTCTesting(int autotest)
{
	...
	u32 check;
	check = ioread32(SQ_RTC_TIME);
	RESET_RTC_CIRCUIT();
	MSDELAY(1000);
	if(ioread32(SQ_RTC_TIME) == check)
        	RESET_RTC_COUNTER();       
	 if (RTC_IS_PWFAIL) {
...
rtc_all_alarm_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(SQ_RTC_DALRM_CD | SQ_RTC_DALRM_CM	| SQ_RTC_DALRM_CY | SQ_RTC_DALRM_CC,
								SQ_RTC_TALRM_CSOS | SQ_RTC_TALRM_CS | SQ_RTC_TALRM_CM | SQ_RTC_TALRM_CH | SQ_RTC_TALRM_CDOW);
...
rtc_sixteen_of_second_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(SQ_RTC_TALRM_CSOS, 0);
...
rtc_second_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(SQ_RTC_TALRM_CS, 0);
...
rtc_minute_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(SQ_RTC_TALRM_CM, 0);
...
rtc_hour_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(SQ_RTC_TALRM_CH, 0);
...
rtc_day_of_week_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(SQ_RTC_TALRM_CDOW, 0);
...
rtc_day_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(0, SQ_RTC_DALRM_CD);
...
rtc_month_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(0, SQ_RTC_DALRM_CM);
...
rtc_year_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(0, SQ_RTC_DALRM_CY);
...
rtc_cenury_test(int autotest)
{
	...
	result = rtc_sub_alarm_test(0, SQ_RTC_DALRM_CC);
...
rtc_sub_alarm_test(u32_t f_time, u32_t f_date)
{
	...
	if (f_time & SQ_RTC_TALRM_CSOS)
		printf("\"Sixteen of Second\" ");
	if (f_time & SQ_RTC_TALRM_CS)
		printf("\"Second\" ");
	if (f_time & SQ_RTC_TALRM_CM)
		printf("\"Minute\" ");
	if (f_time & SQ_RTC_TALRM_CH)
		printf("\"Hour\" ");
	if (f_time & SQ_RTC_TALRM_CDOW)
		printf("\"Day of Week\"");
	printf("\n");	

	printf("Alarm Date on: ");
	if (f_date & SQ_RTC_DALRM_CD)
		printf("\"Day\" ");
	if (f_date & SQ_RTC_DALRM_CM)
		printf("\"Month\" ");
	if (f_date & SQ_RTC_DALRM_CY)
		printf("\"Year\" ");
	if (f_date & SQ_RTC_DALRM_CC)
		printf("\"Century\"");
	printf("\n");
...
rtc_get_time_alarm_test(int autotest)
{
	...
	printf("Alarm Time on: ");
	if (flag & SQ_RTC_TALRM_CSOS)
		printf("\"Sixteen of Second\" ");
	if (flag & SQ_RTC_TALRM_CS)
		printf("\"Second\" ");
	if (flag & SQ_RTC_TALRM_CM)
		printf("\"Minute\" ");
	if (flag & SQ_RTC_TALRM_CH)
		printf("\"Hour\" ");
	if (flag & SQ_RTC_TALRM_CDOW)
		printf("\"Day of Week\"");
...
rtc_get_date_alarm_test(int autotest)
{
	...
	printf("Alarm Date on: ");
	if (flag & SQ_RTC_DALRM_CD)
		printf("\"Day\" ");
	if (flag & SQ_RTC_DALRM_CM)
		printf("\"Month\" ");
	if (flag & SQ_RTC_DALRM_CY)
		printf("\"Year\" ");
	if (flag & SQ_RTC_DALRM_CC)
		printf("\"Century\"");
...
{
	tm->sos      = (temp >> SQ_RTC_TIME_SOS) & BIT_MASK(4);
	tm->sec      = (temp >> SQ_RTC_TIME_S)   & BIT_MASK(4);
	tm->ten_sec  = (temp >> SQ_RTC_TIME_TS)  & BIT_MASK(3);
	tm->min      = (temp >> SQ_RTC_TIME_M)   & BIT_MASK(4);
	tm->ten_min  = (temp >> SQ_RTC_TIME_TM)  & BIT_MASK(3);
	tm->hr       = (temp >> SQ_RTC_TIME_H)   & BIT_MASK(4);
	tm->ten_hr   = (temp >> SQ_RTC_TIME_TH)  & BIT_MASK(2);
	tm->dow      = (temp >> SQ_RTC_TIME_DOW) & BIT_MASK(3);
}
...
{
	dt->day      = (temp >> SQ_RTC_DATE_D)   & BIT_MASK(4);
	dt->ten_day  = (temp >> SQ_RTC_DATE_TD)  & BIT_MASK(2);
	dt->mth      = (temp >> SQ_RTC_DATE_M)   & BIT_MASK(4);
	dt->ten_mth  = (temp >> SQ_RTC_DATE_TM)  & BIT_MASK(1);
	dt->yr       = (temp >> SQ_RTC_DATE_Y)   & BIT_MASK(4);
	dt->ten_yr   = (temp >> SQ_RTC_DATE_TY)  & BIT_MASK(4);
	dt->cent     = (temp >> SQ_RTC_DATE_C)   & BIT_MASK(4);
	dt->ten_cent = (temp >> SQ_RTC_DATE_TC)  & BIT_MASK(4);
}
...
rtc_time_to_u32(const struct rtc_time* tm)
{
	...
	temp |= (tm->dow         << SQ_RTC_TIME_DOW);
	temp |= (tm->ten_hr      << SQ_RTC_TIME_TH );
	temp |= (tm->hr          << SQ_RTC_TIME_H  );
	temp |= (tm->ten_min     << SQ_RTC_TIME_TM );
	temp |= (tm->min         << SQ_RTC_TIME_M  );
	temp |= (tm->ten_sec     << SQ_RTC_TIME_TS );
	temp |= (tm->sec         << SQ_RTC_TIME_S  );
	temp |= (tm->sos         << SQ_RTC_TIME_SOS);
...
rtc_date_to_u32(const struct rtc_date* dt)
{
	...
	temp |= (dt->day      << SQ_RTC_DATE_D );
	temp |= (dt->ten_day  << SQ_RTC_DATE_TD);
	temp |= (dt->mth      << SQ_RTC_DATE_M );
	temp |= (dt->ten_mth  << SQ_RTC_DATE_TM);
	temp |= (dt->yr       << SQ_RTC_DATE_Y );
	temp |= (dt->ten_yr   << SQ_RTC_DATE_TY);
	temp |= (dt->cent     << SQ_RTC_DATE_C );
	temp |= (dt->ten_cent << SQ_RTC_DATE_TC);
...
{
	u32_t temp = readw(SQ_RTC_TALRM);
	rtc_u32_to_time(tm, temp);
	*flag = temp & (SQ_RTC_TALRM_CSOS	|
			SQ_RTC_TALRM_CS	|
			SQ_RTC_TALRM_CM	|
			SQ_RTC_TALRM_CH	|
			SQ_RTC_TALRM_CDOW);
...
{
	u32_t temp = readw(SQ_RTC_DALRM);
	rtc_u32_to_date(dt, temp);
	*flag = temp & (SQ_RTC_DALRM_CD	|
			SQ_RTC_DALRM_CM	|
			SQ_RTC_DALRM_CY	|
			SQ_RTC_DALRM_CC);
}
