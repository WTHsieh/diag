...
#ifdef CONFIG_SCDK
	{	
  	extern struct test_item sq_gpio_main_test_items[];
  	//read scu to get amba mode
  	if(ioread32(SQ_APB0_SCU + 0x28) & SCU_AHB_MODE)
			sq_gpio_main_test_items[1].enable=1;
		else
			sq_gpio_main_test_items[1].enable=0;
	}
#endif
...
sq_gpio_onboard_test(int autotest)
{
	...
	{	
  	extern struct test_item sq_gpio_int_test_items[];
		sq_gpio_int_test_items[1].enable=0;
		sq_set_gpio_base(0, SQ_APB0_GPIO0);
	}
#endif
...
#ifdef CONFIG_SCDK
	sq_set_gpio_base(0, SQ_APB0_MP);
#ifdef SQ_GPIO_WITH_INT
	sq_set_gpio_irq(0, SQ_INTC_MPS0);
#endif
#endif
	ret = test_item_ctrl(&sq_gpio_mode_container, autotest);
	return ret;
