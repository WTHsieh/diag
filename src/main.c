...
#endif
#define DEVICES_POWER_ON()		sq_gpio_set_value_with_mask(PA, SHIFT_MASK(0), SHIFT_MASK(0))	// PA0 = 1
#define DEVICES_POWER_OFF()	sq_gpio_set_value_with_mask(PA, 0, SHIFT_MASK(0))				// PA0 = 0

#define STANDBY_LED_ON()		sq_gpio_set_value_with_mask(PA, SHIFT_MASK(3), SHIFT_MASK(3))	// PA3 = 1
#define STANDBY_LED_OFF()		sq_gpio_set_value_with_mask(PA, 0, SHIFT_MASK(3))				// PA3 = 0

#define OSC_ENABLE()			sq_gpio_set_value_with_mask(PA, SHIFT_MASK(2), SHIFT_MASK(2))	// PA2 = 1
#define OSC_DISABLE()			sq_gpio_set_value_with_mask(PA, 0, SHIFT_MASK(2))				// PA2 = 0

#define DEVICES_RESET_ON()		sq_gpio_set_value_with_mask(PA, 0, SHIFT_MASK(1))				// PA1 = 0
#define DEVICES_RESET_OFF()		sq_gpio_set_value_with_mask(PA, SHIFT_MASK(1), SHIFT_MASK(1))	// PA1 = 1

#define FM_VIO_POWER_ON()		sq_gpio_set_value_with_mask(PE, SHIFT_MASK(3), SHIFT_MASK(3))	// PE3 = 1
#define FM_VIO_POWER_OFF()		sq_gpio_set_value_with_mask(PE, 0, SHIFT_MASK(3))				// PE3 = 0

#define FM_RESET_ON()			sq_gpio_set_value_with_mask(PE, 0x08, 0x08)						// PE4 = 0
#define FM_RESET_OFF()			sq_gpio_set_value_with_mask(PE, 0x18, 0x18)						// PE4 = 1
//#define FM_RESET_ON()			sq_gpio_set_value_with_mask(PE, 0, SHIFT_MASK(4))				// PE4 = 0
//#define FM_RESET_OFF()			sq_gpio_set_value_with_mask(PE, SHIFT_MASK(4), SHIFT_MASK(4))	// PE4 = 1
...
#ifdef CONFIG_MP_GPIO
	#ifdef CONFIG_SCDK
		if(!(ioread32(SQ_APB0_SCU + 0x28) & SCU_AHB_MODE)) //not amba mode
			sq_mp_gpio_init_irq();
	#else
		sq_mp_gpio_init_irq();
	#endif
...
TestEntry(void)
{
	int result = 0;
		...
		sq_scu_upll_normal();
        	...
        	//sq_scu_dev_enable(SCU_DEVCON_UART_HDMA23);   //for test
	#endif

	ns16550_init(SQ_UART_EXT_CLK/16/115200);	//uart initial
#endif
	...
	cpu_clk_mhz = sq_get_cpu_clock() / (1000 * 1000);

	result = test_item_ctrl(&main_test_item_container, 0);
	
	printf("\n\nExit the Sq Diagnostic Program!!\n\n");
