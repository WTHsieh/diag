 ...
 *  */
static inline void
sq_reg_write(u32 reg, u32 value, u32 base)
{
	iowrite32(value, base+reg);
}

static int panther7_hdma_request(u32 ch, struct sq_dma *dma);
static void panther7_hdma_free(u32 ch, struct sq_dma *dma);
static void panther7_hdma_enable(u32 ch, struct sq_dma *dma);
static void panther7_hdma_disable(u32 ch, struct sq_dma *dma);
static void panther7_hdma_set_page_number(u32 ch, struct sq_dma *dma, u32 num);
static void panther7_hdma_isr(void *dma);
static void panther7_hdma_isr_0(void *dma);
static void panther7_hdma_isr_1(void *dma);
static void panther7_hdma_set_count(struct sq_dma *dma);
...
struct sq_dma_ops panther7_hdma_ops = {
...
struct sq_dma panther7_hdma_channel_0 = {
...
struct sq_dma panther7_hdma_channel_1 = {
...
static int
panther7_hdma_request(u32 ch, struct sq_dma *dma)
{
	int ret = 0;
...
static void 
panther7_hdma_free(u32 ch, struct sq_dma *dma)
{
	panther7_hdma_free_irq(dma->irq, dma);
}

static void 
panther7_hdma_enable(u32 ch, struct sq_dma *dma)
{
	u32 inter_ch = *((u32 *)dma->private_data);
	u32 tmp, data_size;
	/* inital setting */
	...
	case 1:
		...
		sq_reg_write(PANTHER7_HDMA_ISRC1, dma->src_addr , dma->base_addr);
		sq_reg_write(PANTHER7_HDMA_IDST1, dma->dst_addr , dma->base_addr);
		panther7_hdma_set_count(dma);
		if (SQ_DMA_MODE_SLICE == dma->mode) {
			sq_reg_write(PANTHER7_HDMA_ISCNT1, dma->slice_cnt-1, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_IADDR_BS1, dma->buf_size-data_size, dma->base_addr);
		}
		sq_reg_write(PANTHER7_HDMA_CON1, conf, dma->base_addr);
		break;
	default:
...
static void 
panther7_hdma_disable(u32 ch, struct sq_dma *dma)
{
	u32 inter_ch = *((u32 *)dma->private_data);
	u32 tmp;
	...
	case 0:
		...
		sq_reg_write(PANTHER7_HDMA_CON0, PANTHER7_HDMA_CPNCNTD_CLR|0x00000000, dma->base_addr);
		...
		sq_reg_write(PANTHER7_HDMA_ISR,
				sq_reg_read(PANTHER7_HDMA_ISR, dma->base_addr) | 
				PANTHER7_HDMA_CH0_INT_MASK |
				PANTHER7_HDMA_CH0_PAGE_INT_MASK |
				PANTHER7_HDMA_CH0_PAGE_ACCUM_OVF_INT_MASK,
				dma->base_addr);

		sq_reg_write(PANTHER7_HDMA_ISRC0, 0, dma->base_addr);
		sq_reg_write(PANTHER7_HDMA_IDST0, 0, dma->base_addr);
		sq_reg_write(PANTHER7_HDMA_ICNT0, 0, dma->base_addr);
		if (SQ_DMA_MODE_SLICE == dma->mode) {
			sq_reg_write(PANTHER7_HDMA_ISCNT0, 0, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_IPNCNT0, 0, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_IADDR_BS0, 0, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_PACNT0, 0, dma->base_addr);
		}
		break;
	case 1:
		/* Clear channel 1configuration register */
		sq_reg_write(PANTHER7_HDMA_CON1, PANTHER7_HDMA_CPNCNTD_CLR|0x00000000, dma->base_addr);
		...
		sq_reg_write(PANTHER7_HDMA_ISRC1, 0, dma->base_addr);
		sq_reg_write(PANTHER7_HDMA_IDST1, 0, dma->base_addr);
		sq_reg_write(PANTHER7_HDMA_ICNT1, 0, dma->base_addr);
		if (SQ_DMA_MODE_SLICE == dma->mode) {
			sq_reg_write(PANTHER7_HDMA_ISCNT1, 0, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_IPNCNT1, 0, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_IADDR_BS1, 0, dma->base_addr);
			sq_reg_write(PANTHER7_HDMA_PACNT1, 0, dma->base_addr);
		}
		break;
	default:
		printf("%s: unknown channel number %d\n", dma->dma_name, ch);
...
static void 
panther7_hdma_set_page_number(u32 ch, struct sq_dma *dma, u32 num)
{
	u32 inter_ch = *((u32 *)dma->private_data);
	
	switch (inter_ch) {
	case 0:
		sq_reg_write(PANTHER7_HDMA_IPNCNT0, num, dma->base_addr);
		break;
	case 1:
		sq_reg_write(PANTHER7_HDMA_IPNCNT1, num, dma->base_addr);
		break;
	default:
		printf("%s: unknown channel number %d\n", dma->dma_name, ch);
	}
...
panther7_hdma_isr_0(void *_dma)
{
	...
	int_stat = sq_reg_read(PANTHER7_HDMA_ISR, dma->base_addr);
...
static void 
panther7_hdma_set_count(struct sq_dma *dma)
{
	int burst_val;
	int data_size_val;
	u32 inter_ch = *((u32 *)dma->private_data);
	...
	if (0 == (dma->tx_cnt % (burst_val * data_size_val))) {
		if (0 == inter_ch) 
			sq_reg_write(PANTHER7_HDMA_ICNT0, (dma->tx_cnt/data_size_val)-1, dma->base_addr);
		else
			sq_reg_write(PANTHER7_HDMA_ICNT1, (dma->tx_cnt/data_size_val)-1, dma->base_addr);
	} else
...
{
	struct sq_dma *dma = (struct sq_dma *)data;
	u32 inter_ch = *((u32 *)dma->private_data);
