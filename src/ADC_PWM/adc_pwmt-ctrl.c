...
static void sq_adc_ch_test_init_pwmt(int hrc, int lrc);
...
extern int
sq_adc_test(int autotest)
{
	int ret = 0;

	//20080123 leonid add for ADC clock 
#if defined (CONFIG_PC7210)
	sq_scu_adc_clock_duty_period(0x8);
#elif defined (CONFIG_PDK)
	sq_scu_adc_clock_duty_period(0x1);
#endif	

	p_pwmt = get_sq_pwmt_structure(USE_PWM_NUM);
	if (NULL == p_pwmt) {
	...
	release_sq_pwmt_structure(USE_PWM_NUM);
...
extern int
sq_adc_conv_w_poll(int autotest)
{
	int ret = 0;
...
extern int
sq_adc_conv_w_int(int autotest)
{
	int ret = 0;
...
extern int sq_adc_ch_test_sub(int v_max);

extern int
sq_adc_ch_test_w_pwmt(int autotest)
{
	...
	sq_adc_ch_test_init_pwmt(hrc, lrc);
	MSDELAY(100);		// wait for capacitance charge or discharge (option)
	sq_adc_init();
	v_ref = sq_adc_read(0);		// read ch[0] value as reference
	printf("v_ref = 0x%x\n", v_ref);
...
extern int
sq_adc_ch_test_w_board(int autotest)
{
	int ret = 0;
	
#if defined(CONFIG_PC9002)
	ret = sq_adc_ch_test_sub(BIT_MASK(7));
#elif defined(CONFIG_PC7210)
	ret = sq_adc_ch_test_sub(0xc5);
#else
	ret = sq_adc_ch_test_sub(BIT_MASK(10));
#endif
...
static void
sq_adc_ch_test_init_pwmt(int hrc, int lrc)
{
...
sq_pwm_timer_isr(void *pparam)
{
	...
	if (sq_pwmt_count >= 5)
		pwmt_drv->set_counter(p_pwmt, sq_get_apb_clock()+1);
...
sq_pwmt_timer_mode_test_init(int single_mode)
{
	...
	pwmt_drv->reset(p_pwmt);
	pwmt_drv->write_prescale_factor(p_pwmt, DEFAULT_PRE_SCL);
	pwmt_drv->write_hrc(p_pwmt, sq_get_apb_clock());
	pwmt_drv->write_lrc(p_pwmt, sq_get_apb_clock());
...
//extern int sq_pwm_capture_duty_irq (void);
////static void sq_pwm_capture_isr(void *pparam);
