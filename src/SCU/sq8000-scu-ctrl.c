...
extern struct test_item_container sq_mode_main_container;

extern int
sq8000_scu_test(int autotest)
{
	return test_item_ctrl(&sq_mode_main_container, autotest);
}


extern struct test_item_container sq_sleep_main_container;

extern int
sq_sleep_mode_test(int autotest)
{
	return test_item_ctrl(&sq_sleep_main_container, autotest);
}


extern struct test_item_container sq_stop_main_container;

extern int
sq_stop_mode_test(int autotest)
{
	return test_item_ctrl(&sq_stop_main_container, autotest);
}


extern int
sq_sleep_with_irq_test(int autotest)
{
	u32 check;
	...
	printf("Sleeping... ");
	sq_scu_pw_standbywfi_enable(1);
	cpu_arm926_do_idle();
	sq_scu_pw_standbywfi_enable(0);
	printf("Wake up!\n");
...
extern int
sq_sleep_with_fiq_test(int autotest)
{
	printf("Press FIQ pin to wake up the system!\n");

	printf("Sleeping... ");
	sq_scu_pw_standbywfi_enable(1);
	sq_scu_dev_enable(SQ_DEVCON_EXT_INT0_NFIQ);
	cpu_arm926_do_idle();
	sq_scu_pw_standbywfi_enable(0);
	printf("Wake up!\n");
...
extern int
sq_stop_with_irq_test(int autotest)
{
	u32 check;
...
extern int
sq_stop_with_fiq_test(int autotest)
{
	printf("Press FIQ pin to wake up the system!\n");
...
wake_up_with_rtc_second(void)
{
	...
	rtc_sub_alarm_test(SQ_RTC_TALRM_CS, 0);
}
