...
sq_spi_marvel_wifi(int autotest)
{
	...
	memset(sq_spi_pattern_buf, 0x0, MARVEL_MAX_NUM * MARVEL_CHAR_LEN / 8);
	memset(sq_spi_cmpr_buf, 0x0, MARVEL_MAX_NUM * MARVEL_CHAR_LEN / 8);
	...
	for (i = 0; i < 10; i++) {
		...
		memcpy((char *)&tx_buf[1], sq_spi_pattern_buf, MARVEL_MAX_NUM * MARVEL_CHAR_LEN / 8);
