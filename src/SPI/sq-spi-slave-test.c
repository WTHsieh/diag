...
sq_spi_slave_run(int type)
{
  ...
  sq_spi_write(
	...
	if(type==SLAVE_RESET) {
		...
		memset(sq_spi_pattern_buf, 0,PATTERN_BUF_SIZE );
		memset(sq_spi_cmpr_buf, 0, PATTERN_BUF_SIZE);
		/* Make test pattern */
...
sq_spi_slave_pure_run(void)
{
  ...
  sq_spi_write(
	...
	/* Clear pattern buffer and compare buffer */
	memset(sq_spi_pattern_buf, 0, PATTERN_BUF_SIZE_PURE*2);
	memset(sq_spi_cmpr_buf, 0, PATTERN_BUF_SIZE_PURE*2);
	/* Make test pattern */
	...
	if(sq_spi_get_current_mode() == MODE_CHAR_LEN_8){
		...
		memcpy(&pdata8[1], sq_spi_pattern_buf, PATTERN_BUF_SIZE_PURE);
  	...
  	printf("compare data...\n");
		...
		memcpy((u8 *)&pdata16[1], sq_spi_pattern_buf, PATTERN_BUF_SIZE_PURE*2);
