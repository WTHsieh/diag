...
static inline void
sq_scu_write(u32 value, u32 reg) 
{
	iowrite32(value, SQ_APB0_SCU+reg);
}

static inline u32
sq_scu_read(u32 reg)
{
	return ioread32(SQ_APB0_SCU+reg);
}

static inline void
sq_scu_show(char *info)
{
	printf("%s", info);
}
...
sq_scu_chip_id (void)
{
	...
	tmp = sq_scu_read(SQ_SCU_CID);

	return tmp;
}

static int sq_scu_pll_formula (int m, int n, int od, int xin);
...
static int 
sq_scu_pll_formula (int m, int n, int od, int xin)
{
	u32 clock;
...
sq_scu_upll_set (int clock)
{
	...
	switch(clock){
		case SQ_SCU_UART_CLOCK_176:			
			...
			break;
		default :		
			sq_scu_show("unknow upll clock !!\n");
			return -1;
			break;
	}

	upll = ((sq_scu_read(SQ_SCU_UPLLCON) & ~SCU_UPLLCON_PLL_MASK) | (tmp ));
...
extern void 
sq_scu_upll_normal (void)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_UPLLCON);
	sq_scu_write(tmp & (~SCU_UPLLCON_PLL_PWR_DN) , SQ_SCU_UPLLCON);
	return ;
}
	
extern void 
sq_scu_upll_power_down (void)
{
	u32 tmp;
...
extern int 
sq_scu_upll_status_get (void)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_UPLLCON) & SCU_UPLLCON_PLL_PWR_DN;
...
sq_scu_mpll_set (int clock)
{
	...
	mpll = ((sq_scu_read(SQ_SCU_MPLLCON) & ~SCU_MPLLCON_PLL_MASK) | (tmp ));
...
extern int 
sq_scu_mpll_get (void)
{
	u32 m,n,od;
	u32 mclk;

	mclk = sq_scu_read(SQ_SCU_MPLLCON);
	...
	mclk = sq_scu_pll_formula(m, n, od, MPLL_XIN);
...
extern void 
sq_scu_mpll_normal (void)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_PWMCON);
	sq_scu_write(tmp | SCU_PWMCON_PWR_NOR , SQ_SCU_PWMCON);
	return ;
}
	
extern void 
sq_scu_mpll_power_down (void)
{
	u32 tmp;
...
extern int 
sq_scu_mpll_status_get (void)		//return 1:normal	0:slow
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_PWR_NOR;
...
extern int 
sq_scu_uart_clk_get (int uart)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_MCLKDIV);
	...
	/*	mclk enable/disable	*/
extern int 
sq_scu_mclk_enable (int ip)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_MCLKEN);
	
	switch(ip){
		case SQ_SCU_MCLK_DCM :
			tmp = tmp | SCU_MCLK_DCM;
			break;
		case SQ_SCU_MCLK_STSDR :
			tmp = tmp | SCU_MCLK_STSDR;
			break;
		case SQ_SCU_MCLK_NFC :
			tmp = tmp | SCU_MCLK_NFC;
			break;
		case SQ_SCU_MCLK_HDMA :
			tmp = tmp | SCU_MCLK_HDMA;
			break;
		case SQ_SCU_MCLK_VOP :
			tmp = tmp | SCU_MCLK_VOP;
			break;
		case SQ_SCU_MCLK_UHC0 :
			tmp = tmp | SCU_MCLK_UHC0;
			break;
		case SQ_SCU_MCLK_UDC :
			tmp = tmp | SCU_MCLK_UDC;
			break;
		case SQ_SCU_MCLK_UHC1 :
			tmp = tmp | SCU_MCLK_UHC1;
			break;			
		case SQ_SCU_MCLK_MAC :
			tmp = tmp | SCU_MCLK_MAC;
			break;		
		case SQ_SCU_MCLK_CLCD :
			tmp = tmp | SCU_MCLK_CLCD;
			break;
		case SQ_SCU_MCLK_GPIO3 :
			tmp = tmp | SCU_MCLK_GPIO3;
			break;
		case SQ_SCU_MCLK_GPIO2 :
			tmp = tmp | SCU_MCLK_GPIO2;
			break;
		case SQ_SCU_MCLK_GPIO1 :
			tmp = tmp | SCU_MCLK_GPIO1;
			break;
		case SQ_SCU_MCLK_GPIO0 :
			tmp = tmp | SCU_MCLK_GPIO0;
			break;
		case SQ_SCU_MCLK_ADC :
			tmp = tmp | SCU_MCLK_ADC;
			break;
		case SQ_SCU_MCLK_PWM :
			tmp = tmp | SCU_MCLK_PWM;
			break;
		case SQ_SCU_MCLK_WDT :
			tmp = tmp | SCU_MCLK_WDT;
			break;
		case SQ_SCU_MCLK_RTC :
			tmp = tmp | SCU_MCLK_RTC;
			break;
		case SQ_SCU_MCLK_TIMER :
			tmp = tmp | SCU_MCLK_TIMER;
			break;
		case SQ_SCU_MCLK_SDMMC :
			tmp = tmp | SCU_MCLK_SDMMC;
			break;
		case SQ_SCU_MCLK_I2S :
			tmp = tmp | SCU_MCLK_I2S;
			break;
		case SQ_SCU_MCLK_I2C :
			tmp = tmp | SCU_MCLK_I2C;
			break;
		case SQ_SCU_MCLK_SPI1 :
			tmp = tmp | SCU_MCLK_SPI1;
			break;
		case SQ_SCU_MCLK_SPI0 :
			tmp = tmp | SCU_MCLK_SPI0;
			break;
		case SQ_SCU_MCLK_UART2 :
			tmp = tmp | SCU_MCLK_UART2;
			break;
		case SQ_SCU_MCLK_UART1 :
			tmp = tmp | SCU_MCLK_UART1;
			break;
		case SQ_SCU_MCLK_UART0 :
			tmp = tmp | SCU_MCLK_UART0;
			break;
		default :
			sq_scu_show("unknow IP number\n");
			return -1;
			break;
	}
...
sq_scu_mclk_disable (int ip)
{
	...
	/*	aclk enable/disable	*/
extern int 
sq_scu_aclk_enable (int ip)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_ACLKEN);
	
	switch(ip){
		case SQ_SCU_ACLK_SDC_HARDMACRO :
			tmp = tmp | SCU_ACLK_SDC_HARDMACRO;
			break;
		case SQ_SCU_ACLK_MAC_HARDMACRO :
			tmp = tmp | SCU_ACLK_MAC_HARDMACRO;
			break;
		case SQ_SCU_ACLK_LCDC_HARDMACRO :
			tmp = tmp | SCU_ACLK_LCDC_HARDMACRO;
			break;
		case SQ_SCU_ACLK_UDC_HARDMACRO :
			tmp = tmp | SCU_ACLK_UDC_HARDMACRO;
			break;
		case SQ_SCU_ACLK_MPLL_OSC_CRYSTAL :
			tmp = tmp | SCU_ACLK_MPLL_OSC_CRYSTAL;
			break;
		case SQ_SCU_ACLK_MPLL_OSC_CLK :
			tmp = tmp | SCU_ACLK_MPLL_OSC_CLK;
			break;
		case SQ_SCU_ACLK_ST_SDR :
			tmp = tmp | SCU_ACLK_ST_SDR;
			break;
		case SQ_SCU_ACLK_HS_UART :
			tmp = tmp | SCU_ACLK_HS_UART;
			break;
		case SQ_SCU_ACLK_LCD_VOP :
			tmp = tmp | SCU_ACLK_LCD_VOP;
			break;		
		case SQ_SCU_ACLK_MAC :
			tmp = tmp | SCU_ACLK_MAC;
			break;
		case SQ_SCU_ACLK_UHC0 :
			tmp = tmp | SCU_ACLK_UHC0;
			break;
		case SQ_SCU_ACLK_UHC1 :
			tmp = tmp | SCU_ACLK_UHC1;
			break;
		case SQ_SCU_ACLK_UTMI_UHC1 :
			tmp = tmp | SCU_ACLK_UTMI_UHC1;
			break;
		case SQ_SCU_ACLK_UTMI_UHC0 :
			tmp = tmp | SCU_ACLK_UTMI_UHC0;
			break;
		case SQ_SCU_ACLK_UDC :
			tmp = tmp | SCU_ACLK_UDC;
			break;
		case SQ_SCU_ACLK_ADC :
			tmp = tmp | SCU_ACLK_ADC;
			break;
		case SQ_SCU_ACLK_I2S :
			tmp = tmp | SCU_ACLK_I2S;
			break;
		case SQ_SCU_ACLK_UART2 :
			tmp = tmp | SCU_ACLK_UART2;
			break;
		case SQ_SCU_ACLK_UART1 :
			tmp = tmp | SCU_ACLK_UART1;
			break;
		case SQ_SCU_ACLK_UART0 :
...
sq_scu_pll_lock_period_set (int period)	
{
	...
	tmp = sq_scu_read(SQ_SCU_MCLKDIV) & ~SCU_MCLKDIV_PLL_LOCK_PERIOD_M;
	tmp = tmp | period;
...
sq_scu_adc_clk_div_set (int div)	
{
	...
	tmp = sq_scu_read(SQ_SCU_MCLKDIV) & ~SCU_MCLKDIV_ADC_CLK_DIV_M;
	tmp = tmp | div;
	sq_scu_write(tmp, SQ_SCU_MCLKDIV);
...
/* CPU/AHB clock ratio	*/
extern int 
sq_scu_clock_ratio_set (int ratio)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_MCLKDIV) & ~SCU_MCLKDIV_CLK_RATIO_MASK;
	...
	switch(ratio){
		...
		case SQ_SCU_CLOCK_RATIO_4_1 :
			...
			break;
		default :
			sq_scu_show("unknow ratio value\n");
			return -1;
			break;			
	}
	sq_scu_write(tmp, SQ_SCU_MCLKDIV);
...
sq_scu_sdram_bus_width_status (void)
{
	...
	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_SDR_WIDTH;
...
sq_scu_dcm_mode_status (void)
{
	...
	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_DCM_MODE;
...
extern int 
sq_scu_usb_mode_status (void)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_USB_MODE;

	if(tmp == SCU_PWMCON_USB_MODE_UDC)
		return SQ_SCU_USB_MODE_UDC;
	else
		return SQ_SCU_USB_MODE_UHC;
...
sq_scu_tps_mac_status (void)
{
	...
	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_TPS_MAC;
	...
	/*	RPS MAC status	*/
extern int 
sq_scu_rps_mac_status (void)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_RPS_MAC;
...
sq_scu_boot_source_status (void)
{
	...
	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_BOOT_MODE;
...
extern int 
sq_scu_auto_boot_fail_status (void)
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_PWMCON) & SCU_PWMCON_BOOT_FAIL;
...
extern void 
sq_scu_uhc0_48clock_input_upll(void)
{
	u32 tmp;
...
extern void 
sq_scu_uhc0_48clock_input_otg_phy(void)
{
	u32 tmp;
...
extern void 
sq_scu_uhc1_48clock_input_upll(void)
{
	u32 tmp;
...
extern void 
sq_scu_uhc1_48clock_input_otg_phy(void)
{
	u32 tmp;
...
sq_scu_hdma_req01_uart(int uart)
{
	...
	switch(uart){
		...
		case 2:
			...
			break;
		default :
			sq_scu_show("unknow uart number\n");
			return -1;
			break;			
	}
...
extern void 
sq_scu_info0_set (u32 inf)	
{
	sq_scu_write(inf, SQ_SCU_INFORM0);
			
	return ;
}		

extern u32 
sq_scu_info0_get ()						//return information0 value
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_INFORM0);
...
/*	SCU_INFORM1	*/
	/*	User defined information register	*/
extern void 
sq_scu_info1_set (u32 inf)
{
	sq_scu_write(inf, SQ_SCU_INFORM1);
			
	return ;
}		

extern u32 
sq_scu_info1_get ()						//return information1 value		
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_INFORM1);
...
/*	SCU_INFORM2	*/
	/*	User defined information register	*/
extern void 
sq_scu_info2_set (u32 inf)
{
	sq_scu_write(inf, SQ_SCU_INFORM2);
			
	return ;
}		

extern u32 
sq_scu_info2_get ()						//return information2 value
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_INFORM2);
...
/*	SCU_INFORM3	*/
	/*	User defined information register	*/
extern void 
sq_scu_info3_set (u32 inf)
{
	sq_scu_write(inf, SQ_SCU_INFORM3);
			
	return ;
}		
		
extern u32 
sq_scu_info3_get ()						//return information3 value	
{
	u32 tmp;

	tmp = sq_scu_read(SQ_SCU_INFORM3);
...
sq_get_cpu_clock (void)
{	
	/*	get power mode */
	if(0 == sq_scu_mpll_status_get())
		sq_clock.cpu_clock = MPLL_XIN;	
	else
		sq_clock.cpu_clock = sq_scu_mpll_get();
				
	return sq_clock.cpu_clock ;
}

extern unsigned long
sq_get_ahb_clock (void)
{
	int ratio;
	
	ratio = sq_scu_clock_ratio_get();

	sq_clock.ahb_clock = sq_get_cpu_clock() / ratio;
	
	return sq_clock.ahb_clock;
}

extern unsigned long
sq_get_apb_clock (void)
{
	sq_clock.apb_clock = sq_get_cpu_clock() / 2;
	
	return sq_clock.apb_clock;
}
...
extern unsigned long
sq_get_uart_clock (int uart)
{
	int tmp;
	int div;

	/*	get power mode */
	tmp = sq_scu_uart_clk_get(uart);
	switch(tmp){
		case SQ_SCU_UART_CLK_24 :
			sq_clock.uart_clock = MPLL_XIN;
			return sq_clock.uart_clock;
			break;
	...
	sq_clock.uart_clock = sq_scu_upll_get() / div;
